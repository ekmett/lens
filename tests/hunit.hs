...
case_read_state_record_field = do
...
case_read_state_record_field_and_apply_function =
  ...
  where
    test = do
      use $ points . to last .  to (vectorFrom origin) . x
...
case_increment_state_record_field = do
  let trig' = trig { _box = (trig .: _box)
                            { _low = (trig .: _box .: _low)
                                     { _y = ((trig .: _box .: _low .: _y) + 1) } } }
  (runState test trig) @?= ((), trig')
...
case_append_to_state_record_field = do
  let trig' = trig { _points = (trig .: _points) ++ [ origin ] }
  (runState test trig) @?= ((), trig')
  where
    test = do
      points ++= [ origin ]
...
case_read_maybe_map_entry =
  (trig ^. labels . at origin)
    @?= Just "Origin"
...
case_read_maybe_state_map_entry =
  ...
  where
    test = do
      use $ labels . at origin

case_read_map_entry =
  (trig ^. labels . valueFor origin)
    @?= "Origin"

case_read_state_map_entry =
  (runState test trig) @?= ("Origin", trig)
  where
    test = do
      use $ labels . valueFor origin

case_modify_map_entry =
  (trig .: labels . valueFor origin %~ List.map toUpper)
