...
withPrism k p = k
  (\b -> fromPrismRight $ mapAlgebraic p (\_ -> PrismRight b) (PrismLeft ()))
  (\s -> case mapAlgebraic p (PrismLeft . fromPrismRight) (PrismRight s) of
    PrismLeft a -> Right a
    PrismRight t -> Left t)
{-# INLINE withPrism #-}
...
prism bt seta f = algebraic $ either pure (fmap bt . runAlgebraic f) . costrength . fmap seta
...
remit p = to $ \b -> runMutator $ mapAlgebraic p (\_ -> Mutator b) Proxy
{-# INLINE remit #-}
...
review p = asks $ \b -> runMutator $ mapAlgebraic p (\_ -> Mutator b) Proxy
{-# INLINE review #-}
...
reviews p tr = asks $ \b -> tr . runMutator $ mapAlgebraic p (\_ -> Mutator b) Proxy
{-# INLINE reviews #-}
...
reuse p = gets $ \b -> runMutator $ mapAlgebraic p (\_ -> Mutator b) Proxy
{-# INLINE reuse #-}
...
reuses p tr = gets $ \b -> tr . runMutator $ mapAlgebraic p (\_ -> Mutator b) Proxy
{-# INLINE reuses #-}
